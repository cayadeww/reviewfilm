...
<script>
    ...
    function addPemeranInput() {
        ...
        newInput.classList.add('mb-4', 'pemeran-input');
        newInput.innerHTML = `
            <input type="text" name="nama_pemeran[]" class="w-full px-4 py-2 border rounded-md" required>
            <button type="button" onclick="removePemeranInput(this)" class="text-red-500">Hapus</button>
        `;
    ...
    document.getElementById('filmPemeranForm').addEventListener('submit', function(event) {
        event.preventDefault();

        let formData = new FormData(this);

        fetch("{{ route('film_pemeran.store') }}", {
            method: "POST",
            headers: {
